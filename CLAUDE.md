...
- `src/Schema/` — Blueprint macros (`jsonIndex`, `jsonWithDefault`, `descIndex`, `forDriver`) and `Schema::forDriver()`; `Family` resolves a connection's family from its schema grammar, `Unsupported::skip()` logs (or throws with `db-portable.strict`).
